...
@section('titulo', 'Editar terminal')

@section('conteudo')
    <form method="POST" action="{{ route('terminais.update', $terminal) }}" class="max-w-3xl space-y-6 rounded-2xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-800 dark:bg-gray-900">
        @csrf
        @method('PUT')
        @include('terminais._form', ['terminal' => $terminal])
