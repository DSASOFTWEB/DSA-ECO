...
| Integrações fiscais (NF-e/NFC-e / NFS-e Nacional) | Emissão hospedagem | Emitente na empresa; NFC-e consumos + NFS-e diárias/serviços na tela da hospedagem (`nfephp-org/sped-nfe`); tabela `cidades` sincronizável via API IBGE (`Gestão → Cidades`) e NCM via Siscomex no estoque | Ajustes de schema SEFAZ/RTC e DANFSe PDF | Homologar com certificado real |
