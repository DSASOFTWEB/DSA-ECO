    ...
    <main class="relative grid min-h-screen lg:grid-cols-2">
        ...
        <section class="flex items-center justify-center px-5 py-16 sm:px-10 lg:px-16">
            <div class="w-full max-w-md">
                ...
                <div class="mb-8">
                    <p class="mb-2 text-sm font-medium text-sky-600 dark:text-sky-400">Bem-vindo de volta</p>
                    <h1 class="text-3xl font-bold tracking-tight text-gray-900 dark:text-white">Entre na sua conta</h1>
                    <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">Acesse o painel de gestão do parque aquático.</p>
                </div>
                ...
                <form method="POST" action="{{ route('login.attempt') }}" class="space-y-5">
                    ...
                    <div>
                        <div class="flex items-center justify-between">
                            ...
                            <a href="{{ route('password.request') }}" class="mb-1.5 text-xs font-medium text-sky-600 hover:underline dark:text-sky-400">Esqueceu a senha?</a>
                        </div>
                    ...
                    <label class="flex cursor-pointer items-center gap-3 text-sm text-gray-600 dark:text-gray-400">
                        <input type="checkbox" name="lembrar" value="1" @checked(old('lembrar')) class="h-4 w-4 rounded border-gray-300 text-sky-600 focus:ring-sky-500 dark:border-gray-700 dark:bg-gray-900">
                        <span>Lembrar de mim</span>
                    </label>

                    <button type="submit" class="inline-flex w-full items-center justify-center rounded-lg bg-sky-600 px-4 py-3 text-sm font-semibold text-white shadow-sm transition hover:bg-sky-700 focus-visible:ring-sky-500 disabled:opacity-50">Entrar</button>

                    <p class="text-center text-sm text-gray-500 dark:text-gray-400">Ainda não tem uma conta? <a href="{{ route('cadastro.create') }}" class="font-semibold text-sky-600 hover:underline dark:text-sky-400">Criar conta grátis</a></p>
                </form>
            </div>
        </section>
