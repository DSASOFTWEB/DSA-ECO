    ...
    <main class="relative grid min-h-screen lg:grid-cols-2">
        ...
        <section class="flex items-center justify-center px-5 py-16 sm:px-10 lg:px-16">
            <div class="w-full max-w-md">
                ...
                <div class="mb-8">
                    <p class="mb-2 text-sm font-medium text-sky-600 dark:text-sky-400">Recuperar acesso</p>
                    <h1 class="text-3xl font-bold tracking-tight text-gray-900 dark:text-white">Esqueceu sua senha?</h1>
                    <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">Digite seu e-mail — se ele estiver cadastrado, enviamos um link pra você criar uma senha nova.</p>
                </div>
                ...
                <form method="POST" action="{{ route('password.email') }}" class="space-y-5">
                    ...
                    <button type="submit" class="inline-flex w-full items-center justify-center rounded-lg bg-sky-600 px-4 py-3 text-sm font-semibold text-white shadow-sm transition hover:bg-sky-700 focus-visible:ring-sky-500 disabled:opacity-50">Enviar link de recuperação</button>

                    <p class="text-center text-sm text-gray-500 dark:text-gray-400"><a href="{{ route('login') }}" class="font-semibold text-sky-600 hover:underline dark:text-sky-400">&larr; Voltar para o login</a></p>
                </form>
            </div>
        </section>
