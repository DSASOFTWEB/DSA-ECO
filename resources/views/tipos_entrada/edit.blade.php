...
@section('titulo', 'Editar tipo de entrada')

@section('conteudo')
    <form method="POST" action="{{ route('tipos-entrada.update', $tipoEntrada) }}" class="max-w-3xl space-y-6 rounded-2xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-800 dark:bg-white/[0.03] sm:p-7">
        @csrf
        @method('PUT')
        @include('tipos_entrada._form', ['tipoEntrada' => $tipoEntrada])
