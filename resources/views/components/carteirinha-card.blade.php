...
<div class="mx-auto w-full max-w-[360px] overflow-hidden rounded-2xl bg-gradient-to-br from-blue-700 via-blue-600 to-sky-500 p-5 text-white shadow-lg" style="aspect-ratio: 1.586 / 1;">
    <div class="flex h-full flex-col justify-between">
        <div class="flex items-start justify-between">
            ...
            <span class="text-[9px] font-semibold uppercase tracking-widest text-white/70">Carteirinha digital</span>
        </div>

        <div class="flex items-center gap-3">
            <div class="grid h-14 w-14 shrink-0 place-items-center overflow-hidden rounded-full border-2 border-white/40 bg-white/15">
                @if ($fotoUrl)
                    <img src="{{ $fotoUrl }}" alt="{{ $titular?->nome }}" class="h-full w-full object-cover">
                @else
                    <span class="text-lg font-bold text-white/90">{{ $iniciais ?: '?' }}</span>
                @endif
            </div>
            <div class="min-w-0">
                <p class="truncate text-base font-bold leading-tight">{{ $titular?->nome ?? 'Titular não encontrado' }}</p>
                <p class="truncate text-xs text-white/75">{{ $ehDependente ? 'Dependente' : 'Titular' }} @if($contratoAtivo) · {{ $contratoAtivo->plano->nome }} @endif</p>
            </div>
        </div>

        <div class="flex items-end justify-between">
            <div>
                <p class="font-mono text-[10px] tracking-wider text-white/70">{{ $carteirinha->codigo }}</p>
                <p class="text-[10px] text-white/70">
                    Validade:
                    {{ $contratoAtivo?->data_fim?->format('m/Y') ?? 'enquanto o plano estiver ativo' }}
                </p>
                <span class="mt-1 inline-flex rounded-full px-2 py-0.5 text-[9px] font-semibold uppercase {{ $carteirinha->estaAtiva() ? 'bg-emerald-400/25 text-emerald-100' : 'bg-rose-400/25 text-rose-100' }}">{{ $carteirinha->status }}</span>
            </div>
