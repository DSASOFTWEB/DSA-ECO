...
| Integrações fiscais (NF-e/NFC-e) | Não iniciado | — | — | **Fora do escopo desta entrega** — arquitetura permite adicionar como um novo `Service` em `Services/Integrations/` seguindo o mesmo padrão de `EvolutionApiService`/`MercadoPagoService`, mas nenhum código de emissão fiscal foi escrito |
