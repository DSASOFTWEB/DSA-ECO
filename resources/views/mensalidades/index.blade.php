...
@section('conteudo')
    ...
    @can('financeiro.enviar_cobranca')
        <div
            x-data="{
                selecionadas: [],
                todasNaPagina: [{{ $mensalidades->filter(fn ($m) => $m->status !== 'pago')->pluck('id')->implode(',') }}],
        ...
        >
            ...
            <div class="mb-3 flex flex-wrap items-center justify-between gap-2 rounded-2xl border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-800 dark:bg-gray-900">
                <span class="text-sm text-slate-500 dark:text-slate-400">
                    <span x-text="selecionadas.length"></span> selecionada(s) para cobrança
                </span>
                <button type="button" @click="enviarLote()" :disabled="!selecionadas.length"
                    class="inline-flex items-center gap-2 rounded-lg bg-emerald-600 px-4 py-2 text-sm font-medium text-white shadow-theme-xs transition hover:bg-emerald-700 disabled:cursor-not-allowed disabled:opacity-40">
                    Enviar cobrança WhatsApp (selecionadas)
                </button>
            </div>
