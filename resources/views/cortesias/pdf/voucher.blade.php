    ...
    @foreach ($acessos as $acesso)
        <div class="ticket">
            ...
            <div class="lado-esquerda">
                <div class="icone">&#10003;</div>
                <div class="rotulo">Entrada<br>Cortesia</div>
            </div>
            ...
            <div class="lado-direita">
                ...
                <div class="nome">{{ $acesso->tipoEntrada?->nome ?? 'Entrada' }}</div>
                <div class="info">{{ $acesso->nomeTitular() ?? 'Convidado' }} &middot; {{ $acesso->unidade->nome }}</div>
                <div class="info">Emitido em {{ $acesso->registrado_em->format('d/m/Y \à\s H:i') }}</div>
                @if ($acesso->validade_ate)
                    <div class="info" style="font-weight: bold; color: #b45309;">Válido até {{ $acesso->validade_ate->format('d/m/Y') }}</div>
                @endif
                ...
                <div class="rodape-ticket">Cortesia &middot; sem valor comercial &middot; acesso #{{ $acesso->id }}</div>
            </div>
        </div>
    @endforeach
</body>
